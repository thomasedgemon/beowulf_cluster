...
func main() {
	...
	if rank == 0 {
		...
		// Wait for done signals from all worker nodes
		for i := 1; i < size; i++ {
			var buf [1]int
			mpi.Recv(&buf, i, 0)
		}
		...
		fmt.Printf("All nodes completed. Total time: %v\n", elapsed)

	} else {
		// Worker node: do work then send "done" signal
		findPrimesParallel(startRange, endRange, numCores)
		var doneSignal = [1]int{1}
		mpi.Send(&doneSignal, 0, 0)
	}
}
